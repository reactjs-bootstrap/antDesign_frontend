...
const UpdatePatient = () => {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/patients/${mrn}`);
        ...
        if (data.success) {
          ...
          form.setFieldsValue({
            ...
            houseNo: patient.address?.[0] || "",
            street: patient.address?.[1] || "",
            city: patient.address?.[2] || "",
            state: patient.address?.[3] || "",
            country: patient.address?.[4] || "",
            pincode: patient.address?.[5] || "",
            dateOfBirth: patient.dateOfBirth
              ? moment(patient.dateOfBirth)
              : null,
            age: patient.age,
            emergencyContactName: patient.emergencyContact?.contactPersonName,
            emergencyContactRelation: patient.emergencyContact?.relation,
            emergencyContactMobile: patient.emergencyContact?.mobileNumber,
            referral: patient.referral || {},
          });
          setReferralMode(patient.referral?.mode || "");
        } else {
